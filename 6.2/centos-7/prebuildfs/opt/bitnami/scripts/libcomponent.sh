...
component_unpack() {
    ...
    if [[ "$name" == "redis-sentinel" ]]; then
        tar --directory "${directory}" --extract --gunzip --file "${base_name}.tar.gz" --no-same-owner --strip-components=2 "redis-sentinel-6.2.7-linux-amd64-centos-7/files/"
    else
